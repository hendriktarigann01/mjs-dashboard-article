...
const Draft = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await api.get(`${endpoint}?status=draft`);
      setData(res.data);
  ...
  const handlePublish = async (id) => {
    ...
    try {
      await api.patch(
        `${activeTab === "project" ? "/projects" : "/news"}/publish/${id}`,
      );
      fetchData();
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Yakin hapus draft ini?")) return;
    setActionLoading(id);
    try {
      const delEndpoint =
        activeTab === "project"
          ...
          <div className="bg-white rounded-b-lg shadow-sm p-2 md:p-6">
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center h-48 text-gray-400">
                <FolderIcon className="w-12 h-12 mb-3 text-gray-200" />
                <p className="text-sm">
                  {searchQuery ? "Tidak ada hasil" : "Tidak ada draft"}
                </p>
              </div>
            ) : (
